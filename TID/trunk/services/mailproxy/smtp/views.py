    ...
    def create(self, request):
        if request.POST.has_key('config') and request.POST.has_key('destination'):
            ...
            if (config.__class__ == {}.__class__) and (destination.__class__ == {}.__class__) and config.has_key("account") and config.has_key("host") and config.has_key("port") and config.has_key("connection") and config.has_key("username") and config.has_key("password") and destination.has_key("subject") and (destination.has_key("message") or destination.has_key("message_html")) and (destination.has_key("to") or destination.has_key("cc") or destination.has_key("bcc")):
                ...
                sended = sendMail(config["account"].encode("utf8"), config["host"].encode("utf8"), int(config["port"]), config["connection"].encode("utf8").upper(), config["username"].encode("utf8"), config["password"].encode("utf8"), destination["subject"].encode("utf8"), message, message_html, to_mails, cc_mails, bcc_mails)
                http_error = 200
