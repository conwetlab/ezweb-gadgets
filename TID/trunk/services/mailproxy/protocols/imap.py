...
class IMAP4Client:
	...
	fetch_response = self.imap.fetch(",".join([str(i) for i in message_list[(begin-1):end]]), "(UID RFC822.SIZE FLAGS BODY[HEADER.FIELDS (DATE FROM TO CC BCC SUBJECT)])")
...
def getFolderList(account, host, port, connection, username, password):
    ...
    if ok:
        ...
        print {"error":error["error"], "message":error["message"], "account": account}
