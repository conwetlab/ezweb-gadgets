...
function dia(dia,mes,anyo,textoCita,modoT){
this.dia = dia;
this.mes = mes;
this.anyo = anyo;
...
/*
diasMarcados [diasMarcados.length] = new dia("11","11","2008","Cumple Antonio","aviso");
diasMarcados [diasMarcados.length] = new dia("10","11","2008","Ir al gimnasio","aviso");
diasMarcados [diasMarcados.length] = new dia("9","11","2008","Partido del Madrid","aviso");
diasMarcados [diasMarcados.length] = new dia("8","11","2008","Clases de yoga","cita");
*/
...
function iniciarPeticion(){
	//alert("peticion");
userAux = EzWebAPI.createRGadgetVariable("user", iniciarPeticionAjax);
}
function iniciarPeticionAjax(){
		//alert("iniciando");
		user = userAux.get();
		 
		feedUrlAux = "http://ezweb.tid.es:8180/appSuite/getMeetings?usuario=" + user + "&ran="+getRandom();
		//alert(feedUrlAux);
...
function displayOk (resp){
	...
	response = resp.responseXML;	
		...
		numeroMensajes = response.getElementsByTagName('cita').length;
	

		 
		
		for (i=0; i<numeroMensajes; i++){
			item = response.getElementsByTagName('cita')[i]; 
			textoCita = item.getElementsByTagName('descripcion')[0].firstChild.data;
			diaFecha = item.getElementsByTagName('dia')[0].firstChild.data;			
			mes = item.getElementsByTagName('mes')[0].firstChild.data;
			anyo = item.getElementsByTagName('anyo')[0].firstChild.data;
			modoT = item.getElementsByTagName('modoT')[0].firstChild.data;
 

			 
			diasMarcados [diasMarcados.length] = new dia(diaFecha,mes,anyo,textoCita,modoT);
...
//alert("error"+ resp.responseXML);
}
 

 

 function isMarked(dia,mes,anyo) {
  ...
  for (i=0;i<diasMarcados.length;i++){
    ...
    actual = diasMarcados[i];
 // alert(actual.anyo + " " + actual.dia + " " + actual.mes +  " " + actual.modoT);
    if (anyo == actual.anyo && dia == actual.dia && mes == actual.mes) {
 		//alert("es" + actual + " "  +actual.modoT) ;
      return actual.modoT;
 ...
 function getCita(dia,mes,anyo) {
  ...
  for (i=0;i<diasMarcados.length;i++){
    ...
    if (anyo == actual.anyo && dia == actual.dia && mes == actual.mes) {
      ...
      textoCitaResult =actual.textoCita;

      }

  }

  return textoCitaResult;
...
RichCalendar.prototype.show = function(x, y, el, position) {
	...
	this.iframe_obj = document.createElement('DIV');
	...
	if (this.is_relative_position(position)) {
		...
		if (typeof(x) == 'number' && typeof(y) == 'number') {
			this.iframe_obj.style.left = 10 + 'px';
			this.iframe_obj.style.top = 58  + 'px';
			//this.iframe_obj.style.align = 'center';	
		}
		this.iframe_obj.style.border = '0px solid #000000';			// TYLOR editted width from 1 to 0
	...
	var iframe_styles = RichCalendar.get_iframe_styles();
	 this.iframe_obj.styles =iframe_styles;
	 ;
	// put calendar content into the iframe
	var iframe_content = '';
 

	this.iframe_doc = document;
	...
	// The awkward 'c' to clear								TYLOR
	this.clear_td = this.head_tr.insertCell(0);
	this.clear_td.innerHTML = '&nbsp;';	// 'c' for clear	TYLOR
	this.clear_td.rc_object_code = 'clear';
	this.clear_td.calendar = this;
//	RichCalendar.attach_events(this.clear_td);				TYLOR
...
// hilight special date
				styleCell =	isMarked(day_num,cur_month,cur_year);
				if (styleCell != "") {
						 
					  cell.rel = getCita(day_num,cur_month,cur_year);
					 cell.onmouseover = toolTip;
 //cell.addeventlistener("onmouseover", alert("hola"), false);
 						 //alert(styleCell);
  
					RichCalendar.add_class(cell, styleCell);
			...
			if (cell.innerHTML != "&nbsp;") {
			 ...
			 Droppables.add(cell.id, { 
    				accept: 'draggable',
   					 hoverclass: 'rc_over',
   					 onDrop: function(elementSource,elementDest) {
						 diaText = elementDest.innerHTML;
						 ...
						 if (tempText =="") {
							...
							$("error").innerHTML="Inserte un texto válido";
							} else {
						 
						 $(elementDest.id).className=elementSource.id ;
						 
						 
						 diasMarcados[diasMarcados.length] = new dia (diaText,mesText,anyoText,tempText,elementSource.id);
						 /// alert("agregado "+ diasMarcados.length);
						 cal_obj.show_date();
							}
						 }
  				});
...
function testBorrarCita(){
if ($("textAreaNuevaCita").value =="") {
	
	//elimino este evento
	 diaText = cal_obj.date.getDate();
	 mesText= cal_obj.date.getMonth();
	anyoText = cal_obj.date.getFullYear();
	deleteCita(diaText,mesText,anyoText);
	
} else {
//modifico este valor
 	diaText = cal_obj.date.getDate();
///	alert("obtengo el dia " + diaText);
	 mesText= cal_obj.date.getMonth();
	anyoText = cal_obj.date.getFullYear();
	 tempText = $("textAreaNuevaCita").value;
	 modificarCita(diaText,mesText,anyoText,tempText);
}
}

function modificarCita(diaText,mesText,anyoText,tempText) {
...
// alert(agregado + diasMarcados.length);
  ...
  for (i=0;i<diasMarcados.length;i++){
    ...
    if (anyoText == actual.anyo && diaText == actual.dia && mesText == actual.mes) {
 
      actual.textoCita = tempText;
		diasMarcados[i] = actual;
		i = diasMarcados.length;
      }

  }
   //envio este valor
   urlEnvio = "http://ezweb.tid.es:8180/appSuite/editMeeting?usuario=" + user + "&ran="+getRandom() + "&dia=" + diaText + "&mes=" + mesText + "&anyo=" + anyoText   + "&descripcion=" + tempText + "&modoT=" + "cita";
		
		EzWebAPI.send_get(urlEnvio, this, nada, nadaMal);
   cal_obj.show_date();
...
function nadaMal(resp){

}

function deleteCita (diaText,mesText,anyoText) {
 ...
 // alert(actual.anyo + " " + actual.dia + " " + actual.mes +  " " + actual.modoT);
    if (anyoText == actual.anyo && diaText == actual.dia && mesText == actual.mes) {
		 //alert("es" + actual + " "  +actual.modoT) ;
   		diasMarcados.splice(i,1);
		
		 
		i = diasMarcados.length;
		//queda eliminar la marca del td representado
		document.getElementById(diaText).className ="";
		document.getElementById(diaText).onmouseover ="";
      }
