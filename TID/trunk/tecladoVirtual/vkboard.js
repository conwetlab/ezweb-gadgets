...
function VKeyboard(container_id, callback_ref, create_arrows, create_updown,
                   create_nav_keys, create_numpad, font_size,
...
VKeyboard.kbArray = [];
VKeyboard.tempCN = null;
VKeyboard.timer = null;
...
VKeyboard.prototype = {
  ...
  _start_flash: function(event)
  {
    ...
    VKeyboard.tempCN = in_el.className;

    if (VKeyboard.tempCN == "keybutton")
      in_el.className="keybuttonpressed";
    else if (VKeyboard.tempCN == "keybuttonctrl")
      in_el.className="keybuttonpressedctrl";

    if (in_el.innerHTML=="BackSpace")
	VKeyboard.timer = window.setInterval(function(e){
		vkb._generic_callback_proc(event);
	},200);
  },
  ...
  _end_flash: function(event)
  {
    ...
    if (VKeyboard.tempCN != null)
      in_el.className=VKeyboard.tempCN;
    VKeyboard.tempCN = null;

    if (VKeyboard.timer!=null)
	window.clearInterval(VKeyboard.timer);
    VKeyboard.timer = null;
  },
  ...
  _setup_key: function(parent, id, top, left, width, height, text_align, line_height, class_name, padding_left, padding_right)
  {
    ...
    key_sub.className = class_name || "keybutton";
  ...
  _construct: function(container_id, callback_ref, create_arrows, create_updown, create_nav_keys, create_numpad,
                       font_size, do_embed, do_gap, start_layout_index)
  {
    ...
    if(!exists || changed)
    {
      ...
      var cn = "keybutton", cns = "keybuttonctrl";
      ...
      for(var i = 0; i < 13; i++)
      {
        this.keys[i] = key = this._setup_key(kb_main, "___key" + String(i), prevY + "px", (prevX + prevW + gap) + "px", cp, cp, c, lh, cn);
      ...
      var kb_kbp = this._setup_key(kb_main, "___kbp", prevY + "px", (prevX + prevW + gap) + "px", (2.96 * cell) + "px", cp, r, lh, cns, "", pad);
      kb_kbp.innerHTML = "BackSpace";
      this.mod[0] = kb_kbp;

      // Top row:

      var kb_tab = this._setup_key(kb_main, "___tab", (prevY + prevH + gap) + "px", gap + "px", (1.48 * cell + gap) + "px", cp, l, lh, cns, pad);
      ...
      var kb_caps = this._setup_key(kb_main, "___caps", (prevY + prevH + gap) + "px", gap + "px", dcell + "px", cp, l, lh, cns, pad);
      ...
      var kb_enter = this._setup_key(kb_main, "___enter_l", prevY + "px", s + "px", (this.kbpH - s) + "px", cp, r, lh, cns, "", pad);
      kb_enter.innerHTML = "Enter";
      this.mod[3] = kb_enter;

      s = this._findX(this.keys[25]) + this._findW(this.keys[25]) + gap;

      var kb_enter_top = this._setup_key(kb_main, "___enter_top", this._findY(kb_tab) + "px", s + "px", (this.kbpH - s) + "px", cx, c, cx, cns);
      kb_enter_top.innerHTML = e;
      kb_enter_top.subst = "Enter";
      // We delete that line of the enter
      kb_enter_top.style.borderBottomColor="rgb(255, 255, 255)";
      ...
      var kb_shift = this._setup_key(kb_main, "___shift", (prevY + prevH + gap) + "px", gap + "px", (2.52 * cell) + "px", cp, l, lh, cns, pad);
      ...
      var kb_shift_r = this._setup_key(kb_main, "___shift_r", prevY + "px", (prevX + prevW + gap) + "px", (this._findX(kb_kbp) + this._findW(kb_kbp) - prevX - prevW - gap) + "px", cp, r, lh, cns, "", pad);
      ...
      var kb_lang = this._setup_key(kb_main, "___lang", (prevY + prevH + gap) + "px", gap + "px", vcell, cp, l, lh, cns, pad);
      this.mod[7] = kb_lang;
      ...
      var kb_res_1 = this._setup_key(kb_main, "___res_1", prevY, (this._findX(kb_lang) + this._findW(kb_lang) + gap) + "px", vcell, cp, c, lh, cns);
      kb_res_1.innerHTML = e;
      this.mod[8] = kb_res_1;

      var kb_res_2 = this._setup_key(kb_main, "___res_2", prevY, (this._findX(kb_res_1) + this._findW(kb_res_1) + gap) + "px", vcell, cp, c, lh, cns);
      kb_res_2.innerHTML = e;
      this.mod[9] = kb_res_2;

      var kb_space = this._setup_key(kb_main, "___space", prevY, (this._findX(kb_res_2) + this._findW(kb_res_2) + gap) + "px", (6.28 * cell) + "px", cp, c,  lh, cns);
      this.mod[10] = kb_space;

      var kb_alt_gr = this._setup_key(kb_main, "___alt_gr", prevY, (this._findX(kb_space) + this._findW(kb_space) + gap) + "px", vcell, cp, c, lh, cns);
      kb_alt_gr.innerHTML = "AltGr";
      this.mod[11] = kb_alt_gr;

      var kb_res_3 = this._setup_key(kb_main, "___res_3", prevY, (this._findX(kb_alt_gr) + this._findW(kb_alt_gr) + gap) + "px", vcell, cp, c, lh, cns);
      kb_res_3.innerHTML = e;
      this.mod[12] = kb_res_3;

      var kb_res_4 = this._setup_key(kb_main, "___res_4", prevY, (this._findX(kb_res_3) + this._findW(kb_res_3) + gap) + "px", vcell, cp, c, lh, cns);
      ...
      {
        var kb_left = this._setup_key(kb_main, "___left", prevY, (this.kbpH + gap + cell / 2) + "px", cp, cp, c, lh, cns);
        kb_left.innerHTML = "&lt;"
        this.mod[14] = kb_left;

        if((create_updown == undefined) ? true : create_updown)
        {
          var kb_down = this._setup_key(kb_main, "___down", prevY, (this._findX(kb_left) + this._findW(kb_left) + gap) + "px", cp, cp, c, lh, cns);
          kb_down.innerHTML = "\\/"
          this.mod[15] = key = kb_down;

          var kb_up = this._setup_key(kb_main, "___up", this._findY(kb_shift_r) + "px", (this._findX(kb_left) + this._findW(kb_left) + gap) + "px", cp, cp, c, lh, cns);
          kb_up.innerHTML = "/\\"
          this.mod[16] = kb_up;
        }
		else key = kb_left;

        var kb_right = this._setup_key(kb_main, "___right", prevY, (this._findX(key) + this._findW(key) + gap) + "px", cp, cp, c, lh, cns);
        kb_right.innerHTML = "&gt;"
  ...
  _create_numpad: function(container_id, parent)
  {
    ...
    var fs = this.fontsize, gap = this.gap;
    ...
    var kb_pad_7 = this._setup_key(parent, "___pad_7", edge_Y, edge, cp, cp, c, lh, cn);
    kb_pad_7.innerHTML = "7";
    this.pad[4] = kb_pad_7;

    var kb_pad_8 = this._setup_key(parent, "___pad_8", edge_Y, edge_1, cp, cp, c, lh, cn);
    kb_pad_8.innerHTML = "8";
    this.pad[5] = kb_pad_8;

    var kb_pad_9 = this._setup_key(parent, "___pad_9", edge_Y, edge_2, cp, cp, c, lh, cn);
    kb_pad_9.innerHTML = "9";
    this.pad[6] = kb_pad_9;

    var kb_pad_plus = this._setup_key(parent, "___pad_plus", edge_Y, edge_3, cp, dp, c, dp2, cn);
    kb_pad_plus.innerHTML = "+";
    this.pad[7] = kb_pad_plus;

    edge_Y = (this._findY(kb_pad_7) + prevH + gap) + "px";

    var kb_pad_4 = this._setup_key(parent, "___pad_4", edge_Y, edge, cp, cp, c, lh, cn);
    kb_pad_4.innerHTML = "4";
    this.pad[8] = kb_pad_4;

    var kb_pad_5 = this._setup_key(parent, "___pad_5", edge_Y, edge_1, cp, cp, c, lh, cn);
    kb_pad_5.innerHTML = "5";
    this.pad[9] = kb_pad_5;

    var kb_pad_6 = this._setup_key(parent, "___pad_6", edge_Y, edge_2, cp, cp, c, lh, cn);
    kb_pad_6.innerHTML = "6";
    this.pad[10] = kb_pad_6;

    edge_Y = (this._findY(kb_pad_4) + prevH + gap) + "px";

    var kb_pad_1 = this._setup_key(parent, "___pad_1", edge_Y, edge, cp, cp, c, lh, cn);
    kb_pad_1.innerHTML = "1";
    this.pad[11] = kb_pad_1;

    var kb_pad_2 = this._setup_key(parent, "___pad_2", edge_Y, edge_1, cp, cp, c, lh, cn);
    kb_pad_2.innerHTML = "2";
    this.pad[12] = kb_pad_2;

    var kb_pad_3 = this._setup_key(parent, "___pad_3", edge_Y, edge_2, cp, cp, c, lh, cn);
    kb_pad_3.innerHTML = "3";
    this.pad[13] = kb_pad_3;

    var kb_pad_enter = this._setup_key(parent, "___pad_enter", edge_Y, edge_3, cp, dp, c, dp2, cns);
    kb_pad_enter.innerHTML = "Enter";
    this.pad[14] = kb_pad_enter;

    edge_Y = (this._findY(kb_pad_1) + prevH + gap) + "px";

    var kb_pad_0 = this._setup_key(parent, "___pad_0", edge_Y, edge, dp, cp, l, lh, cn, 7 * mag + "px");
    kb_pad_0.innerHTML = "0";
    this.pad[15] = kb_pad_0;

    var kb_pad_period = this._setup_key(parent, "___pad_period", edge_Y, edge_2, cp, cp, c, lh, cn);
  ...
  _set_key_state: function(key, on)
  {
    if(key)
    {
      this._detach_event(key, 'mouseup', this._end_flash);
      this._detach_event(key, 'mousedown', this._start_flash);
      this._detach_event(key, 'mouseout', this._end_flash);
      this._detach_event(key, 'click', this._generic_callback_proc);

      if(on){
        this._setup_event(key, 'mouseup', this._end_flash);
        this._setup_event(key, 'mousedown', this._start_flash);
        this._setup_event(key, 'mouseout', this._end_flash);
        this._setup_event(key, 'click', this._generic_callback_proc);

	// Hack! I change the color attribute and to restore the CSS
	// color i set the color to ""
	key.style.color = "";
      } else
	key.style.color = "rgb(180,180,180)";
    }
  },
  ...
  _refresh_layout: function(layout)
  {
    ...
    if(bshift)
    {
      this._set_key_state(shift, true);
      this._set_key_state(shift_r, true);
    }
    else
    {
      this._set_key_state(shift, false);
      this._set_key_state(shift_r, false);
    ...
    if(balt)
    {
      this._set_key_state(alt_gr, true);
    ...
    {
      this._set_key_state(alt_gr, false);
    ...
    if(bcaps && !this.AltGr)
      this._set_key_state(caps, true);
    else
    {
      this._set_key_state(caps, false);
  ...
  _handle_lang_menu: function(event)
  {
    ...
    else
    {
      ...
      {
        var item = vkb._setup_key(menu_main, "___lang_" + String(j), String(h1 * j + 1 ) + "px", "1px", h2, cp, "left", lh, pad-1);
